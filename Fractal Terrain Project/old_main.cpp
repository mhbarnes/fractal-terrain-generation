...
void handleKeyboard(unsigned char key, int x, int y) {
	...
	if (userfocus == VIEW_FOCUS) {
		if (kbinputlen <= KEYBUFFERLEN) {
			...
			//printf("****************%d", key);       
			if ((GLFW_MOD_SHIFT & 2) == 0) {//@?
				shift = true; putchar('S');
			}
			if ((GLFW_MOD_CONTROL & 2) == 0) {
				ctrl = true; putchar('C');
			}
			if ((GLFW_MOD_ALT & 2) == 0) {
				alt = true; putchar('A');
...
int old_main(int argc, char* argv[]) {
	...
	GLFWwindow* window = glfwCreateWindow(resolution.x, resolution.y, "3D Mandelbulb Viewer", NULL, NULL);
	...
	glfwReshapeFunc(handleResolution);
